        ...
        try:
            if file_type == 'xlsx':
                ...
                with io.BytesIO(file_bytes) as f:
                    ...
                    if '库存与周转' in sn:
                        data_pool["df_stock_turnover"] = format_df(
                            pl.read_excel(f, sheet_name='库存与周转').to_pandas())
                    if '国内库存周转' in sn:
                        data_pool["df_country_turnover"] = format_df(
                            pl.read_excel(f, sheet_name='国内库存周转').to_pandas())
                    if '历史海外周转' in sn:
                        data_pool["df_历史海外周转"] = format_df(
                            pl.read_excel(f, sheet_name='历史海外周转').to_pandas())
                    # if 'LT前预测偏差' in sn:
                    #     data_pool["df_LT前预测偏差"] = format_df(
                    #         pl.read_excel(f, sheet_name='LT前预测偏差').to_pandas())
                    if '断货无在途' in sn:
...
with fixed_container:
    ...
    if st.session_state.df_stock_turnover is not None:
        ...
        if filters["sub_market"]: df_stock_turnover_kpi = df_stock_turnover_kpi[
            df_stock_turnover_kpi["子市场"].isin(filters["sub_market"])]
        if filters["category"]: df_stock_turnover_kpi = df_stock_turnover_kpi[
            df_stock_turnover_kpi["品类"].isin(filters["category"])]
        if filters["mrpsku"]: df_stock_turnover_kpi = df_stock_turnover_kpi[
            df_stock_turnover_kpi["主料mrpsku"].isin(filters["mrpsku"])]
        ...
        # if filters["sub_market"]: df_LT前预测偏差 = df_LT前预测偏差[
        #     df_LT前预测偏差["子市场"].isin(filters["sub_market"])]
        ...
        if filters["sub_market"]: df_历史海外周转 = df_历史海外周转[
            df_历史海外周转["子市场"].isin(filters["sub_market"])]
        ...
        with st.container(border=True):
            ...
            #     curr_avg_ltyuce = df_LT前预测偏差[(df_LT前预测偏差['周数'] == last_dt_history)][
            #         '实际周销与预测周销的预测偏差率'].abs().mean()
            # else:
            #     curr_avg_ltyuce = 0
            # ganyu_intervention_rate = df_ganyu_bi["有干预样本数"].sum() / df_ganyu_bi["总样本数"].sum()
            ganyu_intervention_rate = len(
                df_ganyu_kpi[(df_ganyu_kpi['周数'] == last_dt_history) & (df_ganyu_kpi['是否有干预'] == "是")]) / len(
            ...
            c2.metric("海外在库周转", 历史海外在库周转, "目标: P1:45天,P2:30天", delta_color="green",
                      help="(期初在库x单价+期末在库x单价)/2/(周销x单价/7)")
            c3.metric("海外在途周转", 历史海外在途周转, "目标: 60天", delta_color="green",
                      help="(期初在途x单价+期末在途x单价)/2/(周销x单价/7)")
            c4.metric("国内在库周转", f"{历史国内在库周转:.1f}", delta=f"目标: 60天", delta_color="green",
                      help="(国内期初在库x单价+国内期末在库x单价)/2/(周销x单价/7)")
            ...
            c9.metric("干预偏差率(环比)", f"{curr_avg_huanbiganyu:.0%}", delta="目标: 5%", delta_color="green",
                       help="|上周版本对LT后的干预值 - 本周版本对LT后干预值|/本周版本对LT后干预值")
...
def inventorySales_rate_area(df_stock_turnover, df_历史海外周转, curr_filters):
    ...
    df_temp_future['当周周销金额'] = np.minimum(df_temp_future['当周期初在库'] + df_temp_future['当周到货'],
                                                df_temp_future['当周周销']) * df_temp_future['单价']
    df_temp_future_group = df_temp_future.groupby(['周数'])[
        ['当周期初在库', '下周期初在库', '当周期初在途', "下周期初在途", "单价", "期初在库金额", "期末在库金额",
         "期初在途金额", "期末在途金额", "当周到货", "当周周销金额", "当周周销"]].sum().reset_index().sort_values(
        '周数')
    df_temp_future_group['未来海外在库周转'] = (
                ((df_temp_future_group['期初在库金额'] + df_temp_future_group['期末在库金额']) / 2) / (
                    df_temp_future_group['当周周销金额'] / 7)).round(1)
    df_temp_future_group['未来海外在途周转'] = (
                ((df_temp_future_group['期初在途金额'] + df_temp_future_group['期末在途金额']) / 2) / (
                    df_temp_future_group['当周周销金额'] / 7)).round(1)
    ...
    fig.add_hline(y=45, line_dash="dash", line_color="#CC0033", line_width=2, row=1, col=1,
                  annotation_text="P1目标：45天", annotation_position="top right", opacity=0.7,
                  annotation_font=dict(size=10, color="gray"))
    fig.add_hline(y=30, line_dash="dash", line_color="#CC0033", line_width=2, row=1, col=1,
                  annotation_text="P2目标：30天", annotation_position="bottom right", opacity=0.7,
                  annotation_font=dict(size=10, color="gray"))
    fig.add_hline(y=60, line_dash="dash", line_color="#CC0033", line_width=2, row=1, col=2, annotation_text="目标：60天",
                  annotation_position="bottom right", opacity=0.7, annotation_font=dict(size=10, color="gray"))
    # 3. 布局优化
    fig.update_layout(
        height=400,
        showlegend=False,
    ...
    df_future_single['当周周销金额'] = np.minimum(df_future_single['当周期初在库'] + df_future_single['当周到货'],
                                                  df_future_single['当周周销']) * df_future_single['单价']
    df_future_single_oneweek = df_future_single.groupby(['子市场', '品类', '主料mrpsku'])[
        ['当周期初在库', '下周期初在库', '当周期初在途', "下周期初在途", "单价", "期初在库金额", "期末在库金额",
         "期初在途金额", "期末在途金额", "当周到货", "当周周销金额", "当周周销"]].sum().reset_index()
    df_future_single_oneweek['未来海外在库周转'] = (
                ((df_future_single_oneweek['期初在库金额'] + df_future_single_oneweek['期末在库金额']) / 2) / (
                    df_future_single_oneweek['当周周销金额'] / 7)).round(1)
    df_future_single_oneweek['未来海外在途周转'] = (
                ((df_future_single_oneweek['期初在途金额'] + df_future_single_oneweek['期末在途金额']) / 2) / (
                    df_future_single_oneweek['当周周销金额'] / 7)).round(1)
    ...
    if 选择在库下一步动作 != '全部':
        df_future_single_oneweek = df_future_single_oneweek[
            df_future_single_oneweek['在库下一步动作'] == 选择在库下一步动作]
    if 选择在途下一步动作 != '全部':
        df_future_single_oneweek = df_future_single_oneweek[
            df_future_single_oneweek['在途下一步动作'] == 选择在途下一步动作]
    ...
    with stock_col2:
        ...
        st.dataframe(
            df_future_single_oneweek[
                ['子市场', '品类', '主料mrpsku', '未来海外在库周转', '未来海外在途周转', '未来海外在库周转区间',
                 '未来海外在途周转区间', '在库下一步动作', '在途下一步动作']],
            column_config={
                ...
                "未来海外在库周转": st.column_config.NumberColumn("海外在库周转(目标:45天)", format="%d", width=20,
                                                                  alignment="center"),
                "未来海外在途周转": st.column_config.NumberColumn("海外在途周转(目标:60天)", format="%d", width=20,
                                                                  alignment="center"),
...
def predictSales_rate_area(df_yuce, curr_filters):
    ...
    with detail_cat:
        ...
        styled_df = df_cat[['周数', '品类', '单周预测偏差率', '环比预测偏差率', 'SKU个数', '偏差情况']].sort_values(
            "单周预测偏差率", ascending=False).style.map(
    ...
    with detail_sku:
        ...
        df_detail = df_filtered[df_filtered["周数"] == select_week][
            ['周数', '子市场', "channel_name", '品类', "主料mrpsku", "状态", "当周实际值", "当周预测值",
             "单周预测偏差率", "环比预测偏差率"]].copy()
        ...
        styled_df = df_detail[
            ['周数', '子市场', "channel_name", '品类', "主料mrpsku", "状态", "当周实际值", "当周预测值",
             "单周预测偏差率", "环比预测偏差率"]].sort_values("单周预测偏差率", ascending=False).style.map(
            color_deviation, subset=['单周预测偏差率'], target=35
        )
        styled_df = styled_df.map(
            color_deviation, subset=['环比预测偏差率'], target=5
...
def ganyuSales_rate_area(df_ganyu, curr_filters):
    ...
    try:
        ganyu_effect_intervention_rate = len(df_filtered_yes[df_filtered_yes["是否应该干预"] == "是"]) / len(
            df_filtered_yes)
    except ZeroDivisionError:
        ganyu_effect_intervention_rate = 0
    ...
    with col_right:
        ...
        # --- 3. 左侧：精美的圆环图 ---
        business_colors = ['#1f77b4', '#4e79a7', '#76b7b2', '#54a24b', '#e15759', '#f28e2b', '#bab0ac', '#86bcb6',
                           '#dbedff']
    ...
    with col_bias_left:
        ...
        st.plotly_chart(fig_历史干预曲线, width="stretch", height=600)
    ...
    with detail_cat:
        ...
        styled_df = df_cat[['周数', '品类', '单周干预偏差率', '环比干预偏差率', 'SKU个数', '偏差情况']].sort_values(
            "单周干预偏差率", ascending=False).style.map(
    ...
    with detail_sku_ganyu:
        ...
        df_detail = df_filtered_shifouganyu[df_filtered_shifouganyu["周数"] == select_week][
            ['周数', '子市场', "channel_name", '品类', "主料mrpsku", "状态", "当周实际值", "当周干预值",
             "单周干预偏差率", "环比干预偏差率", "是否应该干预"]].copy()
        ...
        styled_df = df_detail[
            ['周数', '子市场', "channel_name", '品类', "主料mrpsku", "状态", "当周实际值", "当周干预值",
             "单周干预偏差率", "环比干预偏差率", "是否应该干预"]].sort_values("单周干预偏差率",
                                                                              ascending=False).style.map(
            color_deviation, subset=['单周干预偏差率'], target=30
        )
        styled_df = styled_df.map(
            color_deviation, subset=['环比干预偏差率'], target=5
...
def delivery_stock_area(df_fahuo, df_历史海外周转, df_断货无在途, curr_filters):
    ...
    with col2:
        ...
        fig_子市场发货.update_yaxes(showgrid=False, zeroline=False, range=[0, (
    ...
    df_filtered_sorted_merge = df_filtered_sorted_merge[[
        "子市场", "主料mrpsku", "品类", "状态", "运输方式", "计划发货量", "配货数量", "配货达成率", "历史海外在库周转",
        "历史海外在途周转", "断货率", "排单数量", "实际出库量", "计划未达成原因"
    ]]
    ...
    with m4:
        ...
        if selected_reason != '全部':
            df_filtered_sorted_merge = df_filtered_sorted_merge[
                df_filtered_sorted_merge['计划未达成原因'] == selected_reason]
    ...
    st.dataframe(
        df_断货无在途_filtered[
            ['子市场', '主料mrpsku', '层级', '品类', '二级品类', '状态', '二级状态', '货源地', '规格', '主料可用库存',
             '替代料可用库存', '待上架库存', 'IQC', '是否有尺寸', '发货计划量']],
        width='stretch',
        height=300,
        column_config={
            "子市场": st.column_config.TextColumn("子市场", width=0.5),
...
if st.session_state.df_fahuo is not None:
    ...
    delivery_stock_area(st.session_state.df_fahuo, st.session_state.df_历史海外周转, st.session_state.df_断货无在途,
                        curr_filters)
    st.divider()
    # 预测指标区域
    st.markdown("# 销")
